...
export function KeywordResultsView({ search }: { search: KeywordSearchDTO }) {
  ...
  return (
    <div className="mx-auto w-full max-w-3xl px-5 pb-24 pt-8 sm:px-8">
      ...
      <div className="mt-6 flex flex-col gap-3 rounded-3xl border border-border bg-card px-6 py-8 text-center sm:px-10">
      ...
      <div className="mt-8 flex flex-wrap items-center justify-between gap-3">
        ...
        <button
          ...
          className="inline-flex items-center gap-1.5 rounded-full border border-border px-3.5 py-2 text-sm font-semibold text-foreground transition-colors hover:border-brand hover:text-brand-strong disabled:cursor-not-allowed disabled:opacity-40"
        >
          {copied ? (
            <>
            ...
            <>
              <Copy className="h-4 w-4" /> Copy saved ({savedCount})
            </>
          )}
        </button>
      </div>
